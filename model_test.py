...
from try_habitat import action, semantic_sensor
...
class DataSampler():
    def __init__(self, split='val'):
        ...
        self.val_set = ['1LXtFkjw3qL']
        self.EDGE_EPISODES = 1#self.args.EDGE_EPISODES # 每个场景采样的次数
    ...
    def display_save(self, rgb_obs, test_scene, semantic_obs=np.array([]), depth_obs=np.array([])):
    ...
    def trajectory_generator(self, test_scene_path):
        ...
        angle = np.deg2rad(0)  # 转换为弧度
        rotation = quat_from_angle_axis(angle, np.array([0, 1, 0]))  # 四元数表示绕Y轴的旋转
        agent_state.rotation = rotation

        agent_state.position = random_navigable_position  # 设置机器人的position
        ...
        # print("action", action)
        for _ in range(360//self.args.turn_amount):
            ...
            x.append(rgb)
            if self.data_save:
                self.display_save(rgb, test_scene_path, semantic, depth)
    ...
    def test_model(self):
        ...
        for test_scene in test_scenes:
            ...
            for _ in range(self.EDGE_EPISODES):
                ...
                img1 = torch.from_numpy(img1.transpose(2,0,1)).unsqueeze(dim=0)
                img2 = torch.from_numpy(img2.transpose(2, 0, 1)).unsqueeze(dim=0)
                # print(img1.shape)
                input_data = torch.concatenate((img1,img2),dim=1).float().to(self.args.device)
                output = self.placenet(input_data)
                # print(output)
                outputs.append(output)

                # x1 = self.trajectory_generator(test_scene_path)  # 生成某个场景的一个轨迹
                # x2 = self.trajectory_generator(test_scene_path)
                # img1 = x1[0]
                # img2 = x2[0]
                #
                # img1 = torch.from_numpy(img1.transpose(2, 0, 1)).unsqueeze(dim=0)
...
if __name__ == '__main__':
    data_sampler = DataSampler(split='val')
    result = data_sampler.test_model()
